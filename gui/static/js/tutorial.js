$(window).load(function() {
  ...
  if (params && params["tutorial"]) {
    ...
    govTour.addStep("add-fromhttp", { 
     ...
     buttons: [ { 
      text: 'Next', 
      action: checkBlockBeforeProgress("fromhttpstream", "type")
    } ],
    ...
    govTour.addStep("edit-fromhttp", { 
     ...
     buttons: [ {
      text: 'Next',
      action: checkBlockBeforeProgress("http://developer.usa.gov/1usagov", "endpoint")
     }],
     attachTo: 'svg'
   });
    ...
    buttons: [{ 
      text: 'Next',
      action: checkConnectionsBeforeProgress("fromhttpstream", "tolog")
    }],
     tetherOptions: { 'targetAttachment': 'bottom right', 'attachment': 'bottom right' },
     attachTo: 'svg'
   });
